...
void convolution2d_streams(float *h_image, float *h_input, float *h_filter) {
     ...
     *
     * The following statement describes how many thread blocks will be created when a kernel is launched
     * when you divide the total amount of work across a number of different streams you will have to adjust this
     * statement. In the y-direction the number of thread blocks created is simply the problem size divided by the 
     * height of a thread block rounded up. In the x-directions it is equal to the problem size divided by the
